...
import {chdir, execl, fork, realpath} from 'unistd'
import {waitpid} from 'sys/wait'
import {PassThrough} from 'stream'

program.usage('[target]').parse([''].concat(arguments))
...
export default async function (parent: string = 'latest', target: string) {
  ...
  try {
    ...
    if (pid === 0) return await execl(path.resolve(bin, 'load'), path.resolve('/record/hash', target))
